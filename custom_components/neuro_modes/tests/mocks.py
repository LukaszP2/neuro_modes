"""Mock setup for pytest - loaded BEFORE conftest.py imports anything."""
import sys
import types


def setup_mocks():
    """Setup all mocked modules in sys.modules."""
    ...
    setattr(vol_mod, "Schema", lambda value: value)
    setattr(vol_mod, "Required", lambda key, default=None: key)
    ...
    switch_mod = _ensure_module("homeassistant.components.switch")
    ...
    binary_sensor_mod = _ensure_module("homeassistant.components.binary_sensor")
    ...
    restore_state_mod = _ensure_module("homeassistant.helpers.restore_state")
    class RestoreEntity:
